...
def fermat_factorization(num: int, time_limit: Optional[int] = None, max_iterations: int = 10**8) -> Tuple[Optional[int], Optional[int], int]:
...
def run_tests(primes: List[int], num_tests: int, time_limit: Optional[int], num_repeats: int = 5) -> List[float]:
    ...
    for _ in range(num_repeats):
        ...
        with ThreadPoolExecutor() as executor:
            results = list(executor.map(lambda _: run_single_test(primes, time_limit), range(num_tests)))
...
def main():
    primes = optimized_sieve_of_eratosthenes(10**6)  # Smaller prime list for faster testing
    num_tests = 25
