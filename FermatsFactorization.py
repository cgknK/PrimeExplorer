...
def run_fermat_factorizations(primes):
    for _ in range(200):  
        ...
        if not check_result(n1, result1_fermats):
            print(f"n1 için Fermat's factorization yanlış: {result1_fermats}, doğru çarpanlar: {p1}, {q1}")
        if not check_result(n1, result1_llm):
            print(f"n1 için Fermat's factorization llm yanlış: {result1_llm}, doğru çarpanlar: {p1}, {q1}")
        
        if not check_result(n2, result2_fermats):
            print(f"n2 için Fermat's factorization yanlış: {result2_fermats}, doğru çarpanlar: {p2}, {q2}")
        if not check_result(n2, result2_llm):
            print(f"n2 için Fermat's factorization llm yanlış: {result2_llm}, doğru çarpanlar: {p2}, {q2}")
