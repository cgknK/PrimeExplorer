...
def cpu_loader(load):
    ...
    try:
        ...
        while time.time() < end_time:
            # Yoğun hesaplamalar yaparak CPU'yu yüke sokma
            for _ in range(load):
                for _ in range(1000):
...
def heat_cpu(duration, load):
    ...
    logging.info(f"CPU'yu {duration} saniye boyunca yük ({load}) ile ısıtma başlatıldı.")

    # CPU izleme thread'i başlatma
    monitor_thread = Thread(target=monitor_cpu, args=(1,))
    monitor_thread.start()

    with concurrent.futures.ThreadPoolExecutor(max_workers=num_workers) as executor:
        futures = [executor.submit(cpu_loader, load) for _ in range(num_workers)]
        ...
        monitor_thread.join()  # İzleme thread'inin tamamlanmasını bekle
...
if __name__ == "__main__":
    setup_logging()
    try:
        heat_cpu(60, 1000)  # 60 saniye boyunca yük (intensity) ile CPU ısıtma
