#math log kullanmak yerine en fazla enerji harcayan komut (belki avx yada diğer simd) araştırılmalı.
import numpy as np
import time
import psutil
...
def avx_matrix_multiply(A, B, duration):
    end_time = time.time() + duration
    while time.time() < end_time:
        C = np.dot(A, B) #muhtemelen avx
    logging.info("Matris çarpımı tamamlandı.")

avx_matrix_multiply(A, B, 60)
