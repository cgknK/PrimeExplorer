"""
CPU affinity ayarı doğru şekilde yapılmadığı için düzeltildi.
Her çekirdeğe özel thread ID kullanımı sağlandı.
İzleme ve yükleme fonksiyonlarının doğru çalışması için düzenlemeler yapıldı.
"""
...
def cpu_loader(load, duration, core_id):
    ...
    try:
        ...
        logging.info(f"Thread {current.name} (Core {core_id}) tamamlandı.")
    except Exception as e:
        logging.error(f"Thread {current.name} (Core {core_id}) hata: {e}")
...
def heat_cpu(duration, load_per_core):
    ...
    with concurrent.futures.ThreadPoolExecutor(max_workers=num_workers) as executor:
        futures = [executor.submit(cpu_loader, load_per_core[i], duration, i) for i in range(num_workers)]
